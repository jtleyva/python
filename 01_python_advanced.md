...
_Decisión clave:_ Se usa `httpx` porque `requests` es sincrónico y bloquearía el event loop de asyncio. Se configura `max_connections` para no saturar los file descriptors ni hacer DoS al servicio de destino.
...
## What is lemmatization and steamming and what are the differences between them?

- **Lemmatization**: Proceso de reducir una palabra a su forma base o lema, teniendo en cuenta el contexto y la parte del discurso. Ejemplo: "running" -> "run".
- **Stemming**: Proceso de reducir una palabra a su raíz o stem, sin tener en cuenta el contexto. Ejemplo: "running" -> "runn".

diferencia clave: La lematización produce palabras reales, mientras que el stemming puede producir palabras no reales.
...
- **Escalabilidad:** Siempre establecer _timeouts_ y usar _Connection Pools_ cuando se hacen peticiones HTTP asíncronas o consultas a DB.
...
- _Asyncio:_ Ideal para alta concurrencia I/O con miles de conexiones (WebSockets, microservicios que llaman a otros microservicios). Trade-off: Require que **todo** tu ecosistema de librerías sea asíncrono (drivers de DB asíncronos, clientes HTTP asíncronos). Código más difícil de debuggear.
- _Threads:_ Ideal cuando usas librerías legadas que bloquean. Trade-off: Consumen más memoria por hilo y el sistema operativo debe gestionar el context switching, limitando la escalabilidad a unos pocos miles concurrentes a lo sumo.
